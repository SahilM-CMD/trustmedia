...
# Paths relative to DeepfakeDetector folder
RAW_ROOT = Path("../data/CASIA2")
OUT_ROOT = Path("../data/objects_processed")

REAL_FOLDER = "Au"   # authentic
FAKE_FOLDER = "Tp"   # tampered
...
def collect_image_paths():
    ...
    fake_root = RAW_ROOT / FAKE_FOLDER
    ...
    print(f"Collected {len(real_paths)} real (Au) images and {len(fake_paths)} fake (Tp) images.")
...
def copy_split(split_name, real_list, fake_list):
    ...
    print(f"[{split_name}] copying {len(real_list)} real, {len(fake_list)} fake...")
...
def main():
    ...
    print("Done. Processed CASIA2 data in:", OUT_ROOT.resolve())
